...
app = FastAPI()
filter_cols = ['eligibility', 'scheme_category', 'tags', 'level']
...
async def filter_schemes_endpoint(request: SchemeFilterRequest):
    ...
    if request.level:
        filtered_df = filtered_df[filtered_df['scheme_category'].str.contains(request.level, case=False, na=False)]
        
    if request.category:
        filtered_df = filtered_df[
            filtered_df['scheme_category'].str.contains(request.category, case=False, na=False) |
